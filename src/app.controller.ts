...
import { MindfulMsPayload, SendEmailOpts } from 'mindful-commons';
import { MindfulLogger } from 'ms-helpers-lib';
...
export class AppController {
  ...
  constructor(
    ...
    if (usernames.length === 0) {
      this.logger.error(
        'Not found config for email.usernames',
        'Out of context',
      );
  ...
  async handleSendEmail(mindfulPayload: MindfulMsPayload<SendEmailOpts>) {
    const loggedUserEmailIfProvided = mindfulPayload.loggedUser
    ...
    try {
      ...
      this.logger.error(
        'Something went wrong when sending email' + e.toString(),
        loggedUserEmailIfProvided,
      );
    }
  }
